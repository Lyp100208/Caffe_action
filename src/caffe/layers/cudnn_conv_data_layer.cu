...
__global__ void sync_conv_data_groups() { }

template <typename Dtype>
void CuDNNConvolutionDataLayer<Dtype>::Forward_gpu(
...
//  LOG(INFO) << "use cuDNN!";
  ...
  //  LOG(INFO) << "fwd_algo: " << fwd_algo_[i];
    ...
    sync_conv_data_groups<<<1, 1>>>();
  }
}

template <typename Dtype>
void CuDNNConvolutionDataLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < top.size(); ++i) {
   // LOG(INFO) << " bwd_filter_algo: " <<  bwd_filter_algo_[i] <<" bwd_data_algo: " << bwd_data_algo_[i];
...
INSTANTIATE_LAYER_GPU_FUNCS(CuDNNConvolutionDataLayer);
