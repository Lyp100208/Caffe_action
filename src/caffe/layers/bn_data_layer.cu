...
void BNDataLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  mean_statistic<Dtype><<<channels_, THREAD_BLOCK_SIZE>>>(num_, height_ * width_, channels_, 
                   Dtype(1. / (height_ * width_ * num_)),this->phase_==TRAIN,
                   this->phase_ == TEST && moving_average_, decay_, Dtype(1) - decay_,
  ...
  var_statistic<Dtype><<<channels_, THREAD_BLOCK_SIZE>>>(num_, height_ * width_, channels_, Dtype(2),
                   ...
                   this->phase_ == TRAIN, this->phase_ == TEST && moving_average_,
                   (num_)*decay_/(num_-1), Dtype(1)-(num_)*decay_/(num_-1),
                   const_top_data, this->blobs_[3]->mutable_gpu_data(), 
                   top_data,x_norm_.mutable_gpu_data(),x_std_.mutable_gpu_data(),scale_data,shift_data);
  CUDA_POST_KERNEL_CHECK;
}
...
void BNDataLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  caffe_gpu_scal(this->blobs_[2]->count(), Dtype(1) / Caffe::getThreadNum(), this->blobs_[2]->mutable_gpu_data());
  caffe_gpu_scal(this->blobs_[3]->count(), Dtype(1) / Caffe::getThreadNum(), this->blobs_[3]->mutable_gpu_data());

#ifdef STRATEGY_0  
  // synchronize
  cudaDeviceSynchronize();
  MPI_Allreduce(MPI_IN_PLACE, scale_diff, this->blobs_[0]->count(), MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
  cudaDeviceSynchronize();
  MPI_Allreduce(MPI_IN_PLACE, shift_diff, this->blobs_[1]->count(), MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
  cudaDeviceSynchronize();
  MPI_Allreduce(MPI_IN_PLACE, this->blobs_[2]->mutable_gpu_data(), this->blobs_[2]->count(), MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
  cudaDeviceSynchronize();
  MPI_Allreduce(MPI_IN_PLACE, this->blobs_[3]->mutable_gpu_data(), this->blobs_[3]->count(), MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
  cudaDeviceSynchronize();
#else
  ...
  task_list->push_back(new MpiTask<Dtype>(NULL,0,this->blobs_[2].get(),0,this->blobs_[2]->count()));
  task_list->push_back(new MpiTask<Dtype>(NULL,0,this->blobs_[3].get(),0,this->blobs_[3]->count()));
  cudaDeviceSynchronize();
#endif
}
