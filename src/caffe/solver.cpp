...
Solver<Dtype>::Solver(const SolverParameter& param)
    : net_(), callbacks_(), requested_early_exit_(false), lr_decay_((Dtype)1.) {
  Init(param);
}
...
//      Caffe::setIter(i);
    ...
    if (request == SolverAction::LR) {
      if (Caffe::getThreadId() == 0) {
        LOG(INFO) << "Base learning rate drop to 0.1";
      }
      this->lr_decay_ *= 0.1;
    }
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  MpiTaskList<Dtype>* task_list = new MpiTaskList<Dtype>();
  ...
  if (param_.snapshot_after_train() && Caffe::getThreadId() == 0 &&
...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    while (request != SolverAction::NONE) {
      if (request == SolverAction::SNAPSHOT
          && Caffe::getThreadId() == 0) {
        Snapshot();
      }
      else if (request == SolverAction::STOP) {
        requested_early_exit_ = true;
      }
  ...
  if (param_.test_compute_loss() && Caffe::getThreadId() == 0) {
  ...
  for (int i = 0; i < test_score.size(); ++i) {
    ...
    if (loss_weight && Caffe::getThreadId() == 0) {
      loss_msg_stream << " (* " << loss_weight
...
void SGDSolver<Dtype>::ApplyUpdate() {
  ...
  if (this->param_.display() && this->iter_ % this->param_.display() == 0 && Caffe::getThreadId() == 0) {
