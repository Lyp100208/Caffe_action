...
class SimpleDataLayer : public BasePrefetchingDataLayer<Dtype> {
 ...
 protected:
  ...
  vector<imageinfo> image_info_;
  int current_row_;

  size_t batch_size_, data_count, elem_size_;
  int crop_size_, final_size_, crop_center_y_offset_;
  Dtype scale_aug_, trans_aug_;
};
...
template <typename Dtype>
class VideoDataRNNLayer : public BasePrefetchingMultilabelDataLayer<Dtype> {
public:
  explicit VideoDataRNNLayer(const LayerParameter& param)
  : BasePrefetchingMultilabelDataLayer<Dtype>(param) {}
  virtual ~VideoDataRNNLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "VideoDataRNN"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
...
//  shared_ptr<Caffe::RNG> prefetch_rng_;
  ...
  int lines_id_;
  Blob<Dtype> label_blob_;
  Blob<Dtype> marker_blob_;
};

template <typename Dtype>
class FileDataLayer : public Layer<Dtype> {
 public:
